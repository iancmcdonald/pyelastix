...
# file patched by Ramon Casero <rcasero@gmail.com>
__version__ = '1.1_patched'
...
def register(moving, fixed, params, exact_params=False, verbose=1):
    """ register(moving, fixed, params, exact_params=False, verbose=1)

    Perform the registration of `moving` to `fixed`, using the given
    parameters. Returns `(moving_deformed, field)`, where `field` is a
    tuple with arrays describing the deformation for each dimension
    (x-y-z order, in world units).

    Parameters:

    * moving (ndarray or file location):
        The moving image (the one to deform).
    * fixed (ndarray or file location):
    ...
    * verbose (int):
    ...
    If `moving` is a list of images, performs a groupwise registration.
    ...
    # Reference image
    refMoving = moving
    if isinstance(moving, (tuple,list)):
        refMoving = moving[0]

    # Check parameters
    if not exact_params:
        params = _compile_params(params, refMoving)
    ...
    if fixed is None:
        ...
        if not isinstance(moving, (tuple,list)):
            raise ValueError('fixed is None, but moving is not a list.')
        #
        ims = moving
        ndim = ims[0].ndim
        ...
        moving = np.zeros(new_shape, ims[0].dtype)
        for i in range(N):
            moving[i] = ims[i]
        # Set parameters
        #params['UseCyclicTransform'] = True # to be chosen by user
        params['FixedImageDimension'] = moving.ndim
        params['MovingImageDimension'] = moving.ndim
    ...
    path_moving, path_fixed = _get_image_paths(moving, fixed)
